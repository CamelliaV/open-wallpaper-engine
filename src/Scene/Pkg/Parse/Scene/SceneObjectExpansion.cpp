...
auto BuildObjectVisibilityInfo(ref<wpscene::SceneDocument>  document,
    ...
    HashMap<std::int32_t, ObjectVisibilityInfo> result;
...
HashSet<std::int32_t> CollectHiddenLinkedSourceIds(ref<wpscene::SceneDocument>  document,
                                                   const HashSet<std::int32_t>& linked_source_ids,
                                                   Option<ref<rstd::json::Map>> user_properties) {
    HashSet<std::int32_t> result;
    ...
    linked_source_ids.iter().for_each([&](ref<std::int32_t> linked_id) {
        ...
        if (! (**info).visible ||
            HasHiddenUserAncestor(static_cast<std::uint32_t>(*linked_id), visibility))
...
bool PrepareSceneObject(T& object, Option<ref<rstd::json::Map>> user_properties,
                        ref<HashSet<std::int32_t>> linked_source_ids, bool force_invisible) {
    ...
    const bool linked =
        ! object.visible && linked_source_ids->contains(static_cast<std::int32_t>(object.id));
...
Vec<SceneObjectVar> FilterSceneObjects(Vec<SceneObjectVar>          decoded,
                                       ...
                                       Option<ref<rstd::json::Map>> user_properties,
                                       ref<HashSet<std::int32_t>>   linked_source_ids) {
    Vec<SceneObjectVar> result;
    result.reserve(decoded.len());
    auto visibility = BuildObjectVisibilityInfo(document, user_properties);
    for (auto& object : decoded) {
        auto force_invisible = [&](std::int32_t id) {
            return HasHiddenUserAncestor(static_cast<std::uint32_t>(id), visibility);
        };
        bool keep = false;
        RSTD_MATCH(object) {
            RSTD_CASE(Container, value) {
...
auto ExpandSceneObjects(ref<wpscene::SceneDocument> document, mut_ref<fs::VFS> vfs,
    ...
    auto objects =
        FilterSceneObjects(rstd::move(decoded),
                           document,
                           user_properties,
                           ref<HashSet<std::int32_t>>::from_raw_parts(rstd::addressof(linked)));
...
array<std::int32_t, 2> ResolveOrthoProjectionExtent(const wpscene::SceneMetadata& metadata,
    ...
    for (usize index {}; index < objects.len(); ++index) {
        ...
        const auto  area  = static_cast<std::int32_t>(image.size[0] * image.size[1]);
        if (area > width * height) {
            width  = static_cast<std::int32_t>(image.size[0]);
            height = static_cast<std::int32_t>(image.size[1]);
        }
    }
    return { width, height };
}
