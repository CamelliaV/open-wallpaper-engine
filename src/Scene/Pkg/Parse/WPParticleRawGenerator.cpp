...
auto ResolveRopeVertexLayout(const SceneVertexArray& vertices, WPGOption option)
    -> RopeVertexLayout {
    const auto attributes = vertices.GetAttrOffsetMap();
    return {
...
void WriteColor(mut_ref<float[]> data, AttrSlot slot, const WPExtractParticle& value) noexcept {
...
void GenParticlePointData(slice<WPExtractInstance> instances, const WPParticleSubSystem& subsystem,
                          WPGOption option, const PointVertexLayout& layout,
...
void GenRopeParticleData(slice<WPExtractInstance> instances, const WPParticleSubSystem& subsystem,
                         WPGOption option, const RopeVertexLayout& layout,
...
void GenRopeTrailSegments(const WPExtractParticle& value, const WPParticleSubSystem& subsystem,
...
void GenRopeTrailData(slice<WPExtractInstance> instances, const WPParticleSubSystem& subsystem,
                      WPGOption option, const RopeVertexLayout& layout, SceneVertexWriter& writer) {
    for (const auto& instance : instances) {
        if (subsystem.InstanceState(instance.instance_index).no_live_particle) continue;
